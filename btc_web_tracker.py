...
Apex Trader Funding $50K rules built-in. Volatile, less-efficient instruments.

APEX RULES:
  ◈ Daily loss limit $1,000 — hard lock on all desks
  ◈ Trailing drawdown $2,500 from equity peak — hard lock
  ◈ Profit target tracker $3,000 (evaluation pass)
  ◈ Min trading days 7 — progress counter
  ◈ Consistency guard — no single day > 40% of total profit
  ◈ Futures proxy on every signal (which Apex contract to execute)

NOTE: SIMULATION ONLY. Paper trades against live data.
No real orders. For research / discipline training only.
"""
...
.badge-apex{background:var(--apex-dim);color:var(--apex);border:1px solid rgba(255,149,0,.3);}
...
.panel-apex{border-top:2px solid var(--apex);}
...
.apex-banner{background:rgba(255,149,0,0.08);border:1px solid rgba(255,149,0,0.3);border-radius:1px;padding:10px 18px;font-family:'Cinzel',serif;font-size:10px;letter-spacing:.12em;color:var(--apex);text-align:center;margin-bottom:16px;}
.apex-clear{background:rgba(26,255,138,0.05);border:1px solid rgba(26,255,138,0.2);border-radius:1px;padding:10px 18px;font-family:'Cinzel',serif;font-size:10px;letter-spacing:.12em;color:var(--emerald);text-align:center;margin-bottom:16px;}
...
.apex-progress-track{background:var(--obsidian4);height:8px;border-radius:4px;overflow:hidden;margin:6px 0 2px;}
.apex-progress-fill{height:100%;border-radius:4px;transition:width 0.3s ease;}
.proxy-tag{display:inline-block;background:rgba(255,149,0,0.10);border:1px solid rgba(255,149,0,0.3);border-radius:1px;padding:2px 8px;font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--apex);letter-spacing:.1em;}
</style>
""", unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════
# APEX TRADER FUNDING — $50K ACCOUNT CONFIG
# ══════════════════════════════════════════════════════════════
APEX_CONFIG = {
    "account_size":       50_000.0,
    "daily_loss_limit":    1_000.0,   # Hard stop — desks lock if breached
    "trailing_drawdown":   2_500.0,   # From highest equity peak ever
    "profit_target":       3_000.0,   # Evaluation pass threshold
    "min_trading_days":        7,     # Unique calendar days with trades
    "consistency_cap_pct":    40,     # No single day > 40% of total profit
    "warn_at_pct":           0.70,    # Start warning at 70% of each limit
    "max_contracts":          10,     # Apex $50K typical max
}

# ══════════════════════════════════════════════════════════════
# MARKETS — Volatile, less-efficient instruments
# ══════════════════════════════════════════════════════════════
MARKETS = {
    "SOL":  {"label":"Solana",    "sub":"SOL / USD",  "stop":0.040, "crypto":True,
             "color":"#9945FF", "emoji":"◎",
             "apex_proxy":"MBT",  "apex_note":"Micro Bitcoin futures · 0.1 BTC/contract"},
    "SOXL": {"label":"Semis 3×", "sub":"SOXL ETF",   "stop":0.030, "crypto":False,
             "color":"#00C4FF", "emoji":"⚡",
             "apex_proxy":"MNQ",  "apex_note":"Micro E-mini Nasdaq · $2/point"},
    "AVAX": {"label":"Avalanche","sub":"AVAX / USD",  "stop":0.045, "crypto":True,
             "color":"#E84142", "emoji":"▲",
             "apex_proxy":"MET",  "apex_note":"Micro Ether futures · 0.1 ETH/contract"},
    "LINK": {"label":"Chainlink","sub":"LINK / USD",  "stop":0.035, "crypto":True,
             "color":"#2A5ADA", "emoji":"⬡",
             "apex_proxy":"MBT",  "apex_note":"Micro Bitcoin futures · closest crypto"},
    "TQQQ": {"label":"Nasdaq 3×","sub":"TQQQ ETF",   "stop":0.025, "crypto":False,
             "color":"#1AFF8A", "emoji":"↑",
             "apex_proxy":"MNQ",  "apex_note":"Micro E-mini Nasdaq · $2/point"},
    "XBI":  {"label":"Biotech",  "sub":"XBI ETF",    "stop":0.020, "crypto":False,
             "color":"#FF6644", "emoji":"🧬",
             "apex_proxy":"RTY",  "apex_note":"Russell 2000 · $5/point micro"},
}

# Maps crypto symbol → CoinGecko + Binance + fallback price
CRYPTO_MAP = {
...
SESSION_TIPS = {
    "TOKYO":    "SOL and AVAX most active. Asian crypto flows favour momentum — watch SOL/AVAX SMT divergence.",
    "LONDON":   "LINK often trends hard at open. SOXL pre-market range sets the day. Tighten stops near $1K daily limit.",
    "NEW YORK": "All six live. TQQQ and SOXL peak volatility. Best hour for XBI biotech catalysts.",
    "OVERLAP":  "Peak liquidity. Decisive moves on all crypto. TQQQ/SOXL compressed then explosive.",
    "OFF-HOURS":"Patience is a position. Review your Apex daily P&L. Never trade near the $1,000 daily limit.",
}

STARTING_BALANCE  = 50_000.0
MAX_BARS_IN_TRADE = 12

# Smart money hour scores (unchanged from v5.2)
SCORE_MAP = {
...
DEFAULT_TRADERS = [
    _make_trader("CONSERVATEUR","◈",
        "Precision entries only — waits for the perfect storm",
        0.005, 2.5, 65, True,
        "I trade once and trade right. One clean entry beats ten mediocre ones.",
        18, 63, 11),
    _make_trader("MOMENTUM","◆",
        "Rides breakouts and trend continuation aggressively",
        0.015, 2.0, 52, False,
        "The trend is my only edge. I follow it until it ends.",
        42, 54, 22),
    _make_trader("CONTRARIAN","◉",
        "Fades extremes — buys oversold, sells overbought",
        0.020, 1.8, 45, False,
        "When others panic, I act. When they chase, I exit.",
        35, 50, 33),
    _make_trader("SCALPER","⬡",
        "High-frequency micro-positions — small wins, tight stops",
        0.008, 1.4, 50, False,
        "A hundred small edges compound into one large fortune.",
        87, 61, 44),
    _make_trader("MACRO","◇",
        "Slow macro thesis trades — holds positions over sessions",
        0.015, 3.5, 60, True,
        "I don't trade noise. I trade the story beneath the price.",
        12, 67, 55),
    _make_trader("ALGOBOT","⬢",
        "Pure systematic — no discretion, rules-only execution",
        0.010, 2.2, 55, False,
        "Emotion is a bug. My only input is the signal.",
        61, 57, 66),
]
...
DEFAULTS = {
    ...
    # Apex state
    "apex_cumulative_pnl":0.0,
    "apex_daily_pnl":0.0,
    "apex_last_day":"",
    "apex_high_water":APEX_CONFIG["account_size"],
    "apex_trading_days":[],
    "apex_locked":False,
    "apex_lock_reason":"",
    "apex_daily_history":[],
...
if not st.session_state["polygon_key"]:
    ...
    <div style='font-family:Cinzel,serif;font-size:4rem;font-weight:900;letter-spacing:.4em;
    color:#fff;margin-bottom:6px'>NIGEL</div>
    <div style='font-family:Cormorant Garamond,serif;font-style:italic;color:#5a5570;
    font-size:1.1rem;letter-spacing:.2em;margin-bottom:4px'>Private Trading Intelligence · v5.3 Apex Edition</div>
    <div style='font-family:JetBrains Mono,monospace;font-size:10px;color:rgba(255,149,0,0.7);
    letter-spacing:.15em;margin-bottom:40px'>$50K APEX TRADER FUNDING · RULES BUILT-IN</div>
    ...
    <div style='font-family:Cinzel,serif;font-size:11px;letter-spacing:.15em;color:#c9a84c;
...
def fetch_crypto_price(cg_id, days=45, fallback=50.0):
    try:
        ...
        return {"closes":closes,"price":closes[-1],
                "chg":(closes[-1]-closes[-2])/closes[-2]*100,"ok":True}
    except Exception:
...
def fetch_binance_live(sym):
    sym_map = {
        "SOL":"SOLUSDT","AVAX":"AVAXUSDT","LINK":"LINKUSDT",
    ...
    try:
        r = requests.get(
            f"https://api.binance.com/api/v3/ticker/24hr?symbol={bsym}",
            timeout=6).json()
...
def detect_regime(closes, highs=None, lows=None):
    ...
    l = lows  or [c*0.995 for c in closes]; n = 14
    dm_plus=[]; dm_minus=[]
    for i in range(1, len(closes)):
    ...
    return {"regime":regime,"adx_lite":adx_lite,
            "bb_width_pct":bb_width_pct,"atr_pct_rank":int(atr_pct_rank)}
...
def detect_divergence(closes, rsi_vals, lookback=10):
        ...
        if prev_c and prev_r:
            ...
            try:
                ...
                if min(c_slice) < prev_low and rsi_at_price_low > prev_rsi_low + 3:
                    ...
                    result["desc"]     = "BULL DIV · Price lower low, RSI did not"
                elif max(c_slice) > prev_hi and rsi_at_price_hi < prev_rsi_hi - 3:
                    result["bear_div"] = True
                    result["desc"]     = "BEAR DIV · Price higher high, RSI did not"
    ...
    if len(closes) >= 26:
        ...
        if len(macd_series) >= lookback*2:
            ...
            p_slice  = closes[-lookback:];      p_prev = closes[-(lookback*2):-lookback]
            if p_prev and m_prev:
                if min(p_slice) < min(p_prev) and min(m_slice) > min(m_prev)+0.0001:
                    result["macd_bull"] = True
                    result["macd_desc"] = "MACD BULL DIV · Momentum diverging up"
                elif max(p_slice) > max(p_prev) and max(m_slice) < max(m_prev)-0.0001:
                    result["macd_bear"] = True
                    result["macd_desc"] = "MACD BEAR DIV · Momentum fading"
    return result
...
def scan_patterns(closes, highs=None, lows=None):
    ...
    if last4_range < 1.5: patterns.append("TIGHT COIL")
    if len(c) > 5:
...
def smart_money_clock():
    ...
    label = ("PEAK" if score >= 85 else "ACTIVE" if score >= 60
             else "MODERATE" if score >= 40 else "LOW")
...
def volatility_regime(closes, window=20):
    ...
    return {"rv":round(rv_now,1),"rv_pct_rank":round(rank,0),
            "forecast":forecast,"rv_5d":round(rv_5d,1)}


# ══════════════════════════════════════════════════════════════
# STATS LAB — Liquidity, KS Test, Order Flow
# ══════════════════════════════════════════════════════════════
...
def fetch_order_flow_imbalance(sym, limit=500):
    ...
    try:
        ...
        if total <= 0: raise ValueError("zero vol")
        flow_ratio   = buy_vol/total
        flow_score   = int((flow_ratio-0.5)*200)
        price_chg_pct= (last_p-first_p)/first_p*100
        momentum     = ((flow_score > 10 and price_chg_pct > 0) or
                        (flow_score < -10 and price_chg_pct < 0))
        ...
        return {"ok":True,"flow_ratio":round(flow_ratio,3),"flow_score":flow_score,
                ...
                "momentum":momentum,"verdict":verdict,"price_chg_pct":round(price_chg_pct,3)}
    except Exception:
        return {"ok":False,"flow_ratio":0.5,"flow_score":0,"buy_vol":0,
...
def find_rejection_blocks(closes, highs=None, lows=None, lookback=20):
    ...
    for i in range(start, len(closes)):
        op  = closes[i-1]; cl = closes[i]; hi = h[i]; lo = l[i]
        rng = hi-lo
        if rng <= 0: continue
        upper_wick = hi-max(op,cl); lower_wick = min(op,cl)-lo; body = abs(cl-op)
        if lower_wick/rng > 0.55 and (cl-lo)/rng > 0.66 and body/rng < 0.4:
        ...
        if upper_wick/rng > 0.55 and (hi-cl)/rng > 0.66 and body/rng < 0.4:
...
def find_liquidity_sweeps(closes, highs=None, lows=None, lookback=15):
    ...
    for i in range(max(5, len(closes)-8), len(closes)):
        prior_h = h[max(0,i-lookback):i]; prior_l = l[max(0,i-lookback):i]
        if not prior_h or not prior_l: continue
        swing_hi = max(prior_h); swing_lo = min(prior_l)
...
def detect_smt_divergence(closes_a, closes_b, name_a, name_b, lookback=10):
    if (not closes_a or not closes_b or
        len(closes_a) < lookback+2 or len(closes_b) < lookback+2):
    ...
    if not a_prev or not b_prev: return {"smt":None,"desc":"","smt_score":0}
    a_hh = max(a_recent)>max(a_prev); b_hh = max(b_recent)>max(b_prev)
    a_ll = min(a_recent)<min(a_prev); b_ll = min(b_recent)<min(b_prev)
    smt=None; desc=""; score=0
    if a_hh and not b_hh:
        smt="BEAR_SMT"; desc=f"{name_a} made new high · {name_b} did not"; score=10
    elif b_hh and not a_hh:
        smt="BEAR_SMT"; desc=f"{name_b} made new high · {name_a} did not"; score=10
    elif a_ll and not b_ll:
        smt="BULL_SMT"; desc=f"{name_a} made new low · {name_b} held"; score=10
    elif b_ll and not a_ll:
        smt="BULL_SMT"; desc=f"{name_b} made new low · {name_a} held"; score=10
...
def compute_full_signal(closes, highs=None, lows=None, volumes=None, rules=None):
    ...
    macd_xu = macd>macd_sl and macd_p<=macd_sl
    macd_xd = macd<macd_sl and macd_p>=macd_sl
    if macd_xu:   long_pts+=4;  reasons.append("MACD bullish crossover")
    elif macd>macd_sl: long_pts+=2
    if macd_xd:   short_pts+=4; reasons.append("MACD bearish crossover")
    ...
    for rule in (rules or []):
        ...
        if rt=="apex_daily_guard":
            ...
            if daily_left < rv:
                rule_block=True; reasons.append(f"⛔ Apex: <${rv:.0f} daily headroom left")
...
def compute_tqs(sig, sm_score, liq_info=None, flow_info=None,
    ...
    if (div.get("bull_div") and div.get("macd_bull")) or (div.get("bear_div") and div.get("macd_bear")):
        pts+=15
    elif any([div.get("bull_div"),div.get("bear_div"),div.get("macd_bull"),div.get("macd_bear")]):
        pts+=8
    ...
    for i in range(start, len(closes)):
        ...
        if not pos and sig.get("conf",0)>=min_conf and not sig.get("rule_block"):
            ...
            if is_buy or is_sell:
                d   = "long" if is_buy else "short"
                sd  = price*stop_mult
                pos = {"dir":d,"entry":price,"stop":price-sd if is_buy else price+sd,
                       "tp":price+sd*rr if is_buy else price-sd*rr,
                       "units":(bal*risk_pct)/max(sd,1e-9)}
    total   = wins+losses
    if total==0: return 0.0
    ret     = (bal-1000)/1000
    dd      = (peak-bal)/peak if peak>0 else 0
    fitness = (wins/total*40)+(max(0,ret)*100*0.4)+(total*1.5)-(dd*30)
...
def adapt_trader(tr):
    ...
    if wr<35 and current<base+12:
        ...
        msg=f"WR {wr:.0f}% normal → drifting toward base ({new})"
    else: return None
...
# ══════════════════════════════════════════════════════════════
# APEX ACCOUNT ENFORCEMENT — $50K RULES
# ══════════════════════════════════════════════════════════════
def check_apex_limits():
    """
    Enforce Apex Trader Funding $50K rules.
    Returns a status dict. Hard-locks all desks if limits breached.
    """
    now_et    = datetime.now(ZoneInfo("America/New_York"))
    today_str = now_et.strftime("%Y-%m-%d")

    # Roll-over daily P&L at midnight ET
    ...
    if apex_equity > hw:
        hw = apex_equity
        st.session_state["apex_high_water"] = hw
    ...
    if daily_pnl <= -APEX_CONFIG["daily_loss_limit"]:
        locked = True
        lock_reason = (f"Daily loss limit hit · ${abs(daily_pnl):,.0f} lost today "
                       f"(limit ${APEX_CONFIG['daily_loss_limit']:,.0f})")
    elif trailing_dd >= APEX_CONFIG["trailing_drawdown"]:
        locked = True
        lock_reason = (f"Trailing drawdown limit hit · ${trailing_dd:,.0f} below peak "
                       f"(limit ${APEX_CONFIG['trailing_drawdown']:,.0f})")
    ...
    if not locked:
        ...
        if daily_used >= APEX_CONFIG["daily_loss_limit"]*warn_at:
            pct = int(daily_used/APEX_CONFIG["daily_loss_limit"]*100)
            warnings.append(f"⚠ Daily loss {pct}% used — ${APEX_CONFIG['daily_loss_limit']-daily_used:.0f} remaining")
        ...
        if len(trading_days) < APEX_CONFIG["min_trading_days"]:
            warnings.append(f"⚠ Only {len(trading_days)}/{APEX_CONFIG['min_trading_days']} trading days — need more unique days")

    # ── Consistency check ───────────────────────────────────
    daily_hist = st.session_state.get("apex_daily_history",[])
    if cum_pnl > 0 and daily_hist:
        best_day = max((d["pnl"] for d in daily_hist if d["pnl"]>0), default=0)
        if best_day > 0 and best_day/cum_pnl*100 > APEX_CONFIG["consistency_cap_pct"]:
            warnings.append(f"⚠ Consistency: best day ({best_day:,.0f}) is >{APEX_CONFIG['consistency_cap_pct']}% of total profit")

    # ── Apply lock to all desks ──────────────────────────────
    ...
    return {
        ...
        "warnings":        warnings,
        "apex_equity":     round(apex_equity, 2),
...
# ══════════════════════════════════════════════════════════════
# TRADER SIMULATION — v5.3 with Apex P&L tracking
# ══════════════════════════════════════════════════════════════
def _apex_record_trade(pnl):
    """Call after every simulated trade close to update Apex account metrics."""
    now_et    = datetime.now(ZoneInfo("America/New_York"))
    today_str = now_et.strftime("%Y-%m-%d")
    if st.session_state.get("apex_last_day","") != today_str:
        st.session_state["apex_daily_pnl"] = 0.0
        st.session_state["apex_last_day"]  = today_str
    st.session_state["apex_cumulative_pnl"] = (
        st.session_state.get("apex_cumulative_pnl",0.0) + pnl)
    st.session_state["apex_daily_pnl"] = (
        st.session_state.get("apex_daily_pnl",0.0) + pnl)
...
def simulate_trader(tr, market_signals):
    ...
    if pos is not None:
        ...
        p   = sig.get("price", pos.get("entry",0)); is_long = pos.get("dir")=="long"
        ...
        if r_mult>=1.0 and not pos.get("trailed"):
            new_s = entry
            if is_long and new_s>stop_price:  pos["stop"]=entry; pos["trailed"]=True; stop_price=entry
            elif not is_long and new_s<stop_price: pos["stop"]=entry; pos["trailed"]=True; stop_price=entry
        if r_mult>=2.0:
            if is_long:
                ts = p-risk_pu*0.7
        ...
        if exit_reason:
            ...
            tr["open_pos"] = None
            _apex_record_trade(pnl)   # ← Apex account tracking
            adapt_trader(tr); state_save()

    if tr.get("open_pos") is not None: return

    # Apex daily-loss guard — don't open new positions if too close to limit
    ...
    for mk, sig in market_signals.items():
        ...
        sd      = p*stop_mult
        stop    = p-sd if is_buy else p+sd
        tp      = p+sd*tr["rr"] if is_buy else p-sd*tr["rr"]
        risk_amt= tr["balance"]*tr["risk_pct"]
        # Apex guard: don't risk more than daily headroom
        risk_amt = min(risk_amt, max(0, daily_left*0.8))
...
def run_diagnostics(sigs, fg_val):
    ...
    return {"per":per,"overall":round(np.mean(all_scores) if all_scores else 0),
            "fg_score":10 if 20<fg_val<80 else 0}
...
NOTES_SYSTEM = """You are Nigel — private wealth trading intelligence. Refined, authoritative, dry wit.
Format: JSON array of 3-4 notes, each with type (watch|buy|sell|info), market (SOL|SOXL|AVAX|LINK|TQQQ|XBI), and text.
Max 2 sentences per note. Always reference the Apex $50K daily limit if a trade looks risky.
Return ONLY valid JSON."""

AI_ANALYST_SYSTEM = """You are Nigel, senior quant at a prop trading desk running an Apex Trader Funding $50K account.
Rules: $1,000 daily loss limit, $2,500 trailing drawdown, $3,000 profit target, 7 minimum trading days.
Markets: SOL, SOXL, AVAX, LINK, TQQQ, XBI — volatile, less-efficient.
Give sharp, specific, actionable analysis. Make clear calls. Under 400 words. Always note which Apex futures proxy to use."""

WHISPER_SYSTEM = """You are Nigel's inner voice — one-line market intuition.
A single elegant observation. Max 20 words. No quotes. Return ONLY the whisper text."""
...
def generate_notes(sigs, sessions):
    if not CLKEY:
        for mk,sig in sigs.items():
            ...
            elif s=="OVERSOLD":   push_note("buy",mk, f"**{m['label']}** flush complete. Wait for one confirming bar before sizing in.")
    ...
    summaries=" | ".join(f"{MARKETS[k]['label']}: {v['signal']} RSI={v['rsi']:.0f} mom={v['mom']:+.1f}%" for k,v in sigs.items())
...
def generate_whisper(sigs):
    ...
    resp,err=call_claude(f"Apex $50K account signals: {signals_str}. Give one whisper.",WHISPER_SYSTEM,CLKEY,60)
    if resp and not err:
...
def build_ai_context(sigs, prices_dict, diag, apex_status, bt_cache=None):
    now_et  = datetime.now(ZoneInfo("America/New_York"))
    lines   = [f"TIME: {now_et.strftime('%H:%M ET')}",
               f"APEX ACCOUNT: ${apex_status['apex_equity']:,.0f} | Daily P&L: ${apex_status['daily_pnl']:+,.0f} | "
               f"Daily limit left: ${APEX_CONFIG['daily_loss_limit']+apex_status['daily_pnl']:,.0f} | "
               f"Trailing DD: ${apex_status['trailing_dd']:,.0f} / $2,500 | "
               f"Profit target: ${apex_status['cumulative_pnl']:,.0f} / $3,000",
               f"STATUS: {'LOCKED' if apex_status['locked'] else 'CLEAR'}",
               f"HEALTH: {diag.get('overall',0)}/100", "", "SIGNALS:"]
    for mk,sig in sigs.items():
        m = MARKETS[mk]
        lines.append(f"  {mk} ({m['apex_proxy']}): {sig['signal']} conf={sig['conf']}% "
                     f"RSI={sig['rsi']} mom={sig['mom']:+.1f}% regime={sig['regime'].get('regime','?')}")
    return "\n".join(lines)
...
def run_backtest_nigel(closes, mk, risk_pct=0.01, rr=2.0, rules=None):
    ...
    for i in range(22, len(closes)):
        ...
        if pos:
            ...
            if hit_sl or hit_tp:
                ep  = pos["tp"] if hit_tp else pos["stop"]
                pnl = (ep-pos["entry"])*pos["units"] if is_long else (pos["entry"]-ep)*pos["units"]
                bal = max(0,bal+pnl); peak=max(peak,bal); sim_daily_pnl+=pnl
        ...
        # Respect simulated daily loss limit
        if sim_daily_pnl <= -APEX_CONFIG["daily_loss_limit"]: continue
        if not pos and sig.get("signal")!="HOLD" and not sig.get("rule_block"):
            ...
            if is_buy or is_sell:
                d   = "long" if is_buy else "short"
                sd  = price*stop_mult; stop=price-sd if is_buy else price+sd
                tp  = price+sd*rr if is_buy else price-sd*rr
                ra  = min(bal*risk_pct, APEX_CONFIG["daily_loss_limit"]+sim_daily_pnl)
                if ra<=0: continue
                pos = {"dir":d,"entry":price,"stop":stop,"tp":tp,"units":ra/max(sd,1e-9)}
        equity.append(bal)
    ...
    return {"mk":mk,"total_pnl":round(tdf["pnl"].sum(),2),
            "return_pct":round((bal-cap)/cap*100,2),"bh":round(bh,2),
            "win_rate":round(wr,1),"total_trades":len(tdf),"wins":len(wins),
            "losses":len(losses),"avg_win":round(avg_w,2),"avg_loss":round(avg_l,2),
    ...
    for tr in TRADERS:
        for t in tr.get("trades",[]):
            rows.append({"Desk":tr["name"],"Market":t.get("market",""),"Direction":t.get("dir",""),
                ...
                "Bars":t.get("bars","—"),"Apex Proxy":MARKETS.get(t.get("market",""),{}).get("apex_proxy","—")})
...
# ══════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown('<div style="font-family:Cinzel,serif;font-weight:900;font-size:1.4rem;letter-spacing:.3em;color:#fff;margin:16px 0 2px">NIGEL</div>', unsafe_allow_html=True)
    st.markdown('<div style="font-family:JetBrains Mono,monospace;font-size:9px;color:rgba(255,149,0,0.7);letter-spacing:.15em;margin-bottom:16px">APEX $50K EDITION</div>', unsafe_allow_html=True)
    restore_ts   = st.session_state.get("__restore_ts__","—")
    state_kb     = PERSIST_PATH.stat().st_size/1024 if PERSIST_PATH.exists() else 0
    st.markdown(f'<div class="persist-banner"><div style="font-size:9px;letter-spacing:.1em">◈ STATE PERSISTENCE</div><div style="font-size:9px;color:#3a3550;margin-top:3px">Restored {restore_ts} · {state_kb:.1f} KB</div></div>', unsafe_allow_html=True)
    ...
    new_account = st.number_input("Account ($)", 5000, 1000000,
        st.session_state.get("account_size",50_000), 1000)
    ...
    if st.button("♻️ Reset Traders"): st.session_state["traders"]=DEFAULT_TRADERS; state_save(); st.rerun()
    if st.button("🔄 Reset Apex P&L"):
        for k in ["apex_cumulative_pnl","apex_daily_pnl","apex_last_day","apex_trading_days",
                  "apex_locked","apex_lock_reason","apex_daily_history"]:
    ...
    if journal_csv:
        st.download_button("⬇ Trade Journal", data=journal_csv,
...
# Apex enforcement (runs every render)
...
flow_per_market = {mk: (fetch_order_flow_imbalance(mk) if mk in CRYPTO_MAP
                        else {"ok":False,"flow_ratio":0.5,"flow_score":0,"buy_vol":0,
                              "sell_vol":0,"momentum":False,"verdict":"N/A","price_chg_pct":0})
                   for mk in SEL}
    ...
    for mk,sig in market_signals.items():
        if sig.get("signal")!="HOLD" and sig.get("conf",0)>=55:
            ...
            st.session_state["signal_feed"].insert(0,{
                ...
                "apex_proxy":MARKETS[mk]["apex_proxy"]})
...
<div class="nigel-masthead">
  ...
  <div style="text-align:right">
    ...
    <div style="margin-top:6px">
      {''.join(f'<span style="display:inline-block;padding:3px 14px;font-family:Cinzel,serif;font-size:10px;font-weight:700;letter-spacing:.12em;margin-right:8px;background:{c}22;color:{c};border:1px solid {c}44">{n}</span>' for n,c in sessions_now)}
    </div>
...
if apex_status["locked"]:
    ...
    for w in apex_status["warnings"]:
        st.markdown(f'<div class="apex-banner"><span class="apex-dot"></span>{w}</div>', unsafe_allow_html=True)
elif apex_status["target_reached"]:
    st.markdown('<div class="apex-clear">★ EVALUATION TARGET REACHED — $3,000 profit achieved. Keep trading to build buffer.</div>', unsafe_allow_html=True)
...
def make_ticker():
    ...
    for mk in SEL:
        ...
        items.append(f'<span class="tick-item"><span class="tick-sym">{mk}</span><span class="tick-px">{fmt_price(mk,p)}</span><span class="{cc}">{arrow} {abs(chg):.2f}%</span><span class="badge {bc}" style="font-size:9px;padding:1px 7px">{d}</span><span class="badge {rc}">{reg[:3]}</span><span style="font-family:JetBrains Mono,monospace;font-size:9px;color:{tc}">TQS {tqs}</span><span class="proxy-tag">{proxy}</span><span class="tick-sep">·</span></span>')
    inner="".join(items)*3
...
tpnl_c="#1aff8a" if total_pnl>=0 else "#ff2d55"
apex_eq=apex_status["apex_equity"]; aeq_c="#1aff8a" if apex_status["cumulative_pnl"]>=0 else "#ff2d55"
daily_pnl_c="#1aff8a" if apex_status["daily_pnl"]>=0 else "#ff2d55"
hc2=st.columns([3,1,1,1,1,1])
with hc2[1]: st.markdown(f'<div style="text-align:center;padding:8px 0"><div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.14em;color:#5a5570">HEALTH</div><div style="font-family:Cinzel,serif;font-size:2.2rem;font-weight:900;color:{hc}">{health}</div></div>', unsafe_allow_html=True)
with hc2[2]: st.markdown(f'<div style="text-align:center;padding:8px 0"><div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.14em;color:#5a5570">FEAR/GREED</div><div style="font-family:Cinzel,serif;font-size:2.2rem;font-weight:900;color:{fg_color}">{fg_val}</div><div style="font-size:10px;color:#5a5570;font-style:italic">{fg_label}</div></div>', unsafe_allow_html=True)
with hc2[3]: st.markdown(f'<div style="text-align:center;padding:8px 0"><div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.14em;color:#5a5570">APEX EQUITY</div><div style="font-family:Cinzel,serif;font-size:2.2rem;font-weight:900;color:{aeq_c}">${apex_eq:,.0f}</div><div style="font-size:10px;color:#5a5570;font-style:italic">today {apex_status["daily_pnl"]:+,.0f}</div></div>', unsafe_allow_html=True)
with hc2[4]: st.markdown(f'<div style="text-align:center;padding:8px 0"><div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.14em;color:#5a5570">TRAIL DD</div><div style="font-family:Cinzel,serif;font-size:2.2rem;font-weight:900;color:{"#ff2d55" if apex_status["trailing_dd_pct"]>70 else "#c9a84c" if apex_status["trailing_dd_pct"]>40 else "#1aff8a"}">${apex_status["trailing_dd"]:,.0f}</div><div style="font-size:10px;color:#5a5570;font-style:italic">/ $2,500</div></div>', unsafe_allow_html=True)
with hc2[5]: st.markdown(f'<div style="text-align:center;padding:8px 0"><div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.14em;color:#5a5570">TO TARGET</div><div style="font-family:Cinzel,serif;font-size:2.2rem;font-weight:900;color:#c9a84c">${max(0,3000-apex_status["cumulative_pnl"]):,.0f}</div><div style="font-size:10px;color:#5a5570;font-style:italic">{apex_status["pct_to_target"]:.0f}% done</div></div>', unsafe_allow_html=True)

st.markdown(f'<div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.15em;color:#5a5570;margin-bottom:10px"><span class="live-dot"></span>LIVE · APEX $50K · SOL · SOXL · AVAX · LINK · TQQQ · XBI · {"✓ CLAUDE" if CLKEY else "FALLBACK"}</div>', unsafe_allow_html=True)
    ...
    with col:
        ...
        <div class="signal-card {card_cls}">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">
            <div>
              <div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.12em;color:#5a5570">{m['sub']}</div>
            ...
            <div style="text-align:right">
              ...
              <div style="margin-top:4px"><span class="proxy-tag">{m['apex_proxy']}</span></div>
            </div>
          </div>
          <div class="sc-price">{fmt_price(mk,p)}</div>
          <div class="{chg_cls}" style="margin:2px 0 8px">{"▲" if chg>=0 else "▼"} {abs(chg):.2f}% today</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin:6px 0;padding:8px;background:#0d0c16;border-radius:1px">
            <div style="text-align:center"><div style="font-family:Cinzel,serif;font-size:1.5rem;font-weight:900;color:{tqs_c};line-height:1">{tqs}</div><div style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.1em;color:#5a5570">TQS</div></div>
            <div style="text-align:center"><div style="font-family:Cinzel,serif;font-size:1rem;font-weight:900;color:{vc};line-height:1.2">{cv["verdict"]}</div><div style="font-family:Cinzel,serif;font-size:7px;letter-spacing:.1em;color:#5a5570">COUNCIL {cv["long"]}L/{cv["short"]}S</div></div>
          </div>
          ...
          {div_html}<div style="margin-top:5px">{pats_html}</div>
...
# ══════════════════════════════════════════════════════════════
with t0:
    st.markdown('<div style="font-family:Cinzel,serif;font-size:10px;letter-spacing:.2em;color:var(--apex);margin-bottom:8px">APEX TRADER FUNDING — $50K ACCOUNT COMPLIANCE</div>', unsafe_allow_html=True)
    st.markdown('<div style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:13px;color:#5a5570;margin-bottom:18px">Real-time rule enforcement · Daily loss · Trailing drawdown · Profit target · Trading day counter.</div>', unsafe_allow_html=True)

    # Big status
    if apex_status["locked"]:
        status_color="#ff2d55"; status_text="LOCKED"; status_sub=apex_status["lock_reason"]
    elif apex_status["target_reached"]:
        status_color="#1aff8a"; status_text="TARGET HIT"; status_sub="$3,000 profit achieved — evaluation passed"
    elif apex_status["warnings"]:
        status_color="#ff9500"; status_text="WARNING"; status_sub=apex_status["warnings"][0]
    else:
        status_color="#1aff8a"; status_text="CLEAR"; status_sub="All limits healthy — trade with discipline"

    a0c1, a0c2 = st.columns([2,3])
    with a0c1:
        st.markdown(f"""
        <div class="panel" style="border-top:3px solid {status_color};text-align:center;padding:32px 20px">
          <div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.2em;color:#5a5570;margin-bottom:12px">ACCOUNT STATUS</div>
          <div style="font-family:Cinzel,serif;font-size:3rem;font-weight:900;color:{status_color};line-height:1">{status_text}</div>
          ...
          <div style="font-family:JetBrains Mono,monospace;font-size:11px;line-height:2;text-align:left">
            <div><span style="color:#5a5570">Apex equity:</span> <span style="color:#fff">${apex_status["apex_equity"]:,.2f}</span></div>
            <div><span style="color:#5a5570">High water:</span> <span style="color:#1aff8a">${apex_status["high_water"]:,.2f}</span></div>
            <div><span style="color:#5a5570">Today's P&L:</span> <span style="color:{"#1aff8a" if apex_status["daily_pnl"]>=0 else "#ff2d55"}">${apex_status["daily_pnl"]:+,.2f}</span></div>
    ...
    with a0c2:
        st.markdown('<div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.15em;color:var(--apex);margin-bottom:12px">LIMIT GAUGES</div>', unsafe_allow_html=True)
        ...
        <div class="panel" style="margin-bottom:10px;border-left:2px solid {dl_color}">
          <div style="display:flex;justify-content:space-between;margin-bottom:6px">
            <span style="font-family:Cinzel,serif;font-size:10px;color:#fff;letter-spacing:.08em">DAILY LOSS LIMIT</span>
          ...
          <div style="font-family:JetBrains Mono,monospace;font-size:10px;color:#5a5570;margin-top:4px">
            ${dl_left:,.0f} remaining today · {"LOCKED — no new trades" if dl_pct>=100 else "CAUTION — reduce size" if dl_pct>=70 else "CLEAR"}
          </div>
        </div>""", unsafe_allow_html=True)
        ...
        <div class="panel" style="margin-bottom:10px;border-left:2px solid {td_color}">
          <div style="display:flex;justify-content:space-between;margin-bottom:6px">
            <span style="font-family:Cinzel,serif;font-size:10px;color:#fff;letter-spacing:.08em">TRAILING DRAWDOWN</span>
          ...
          <div style="font-family:JetBrains Mono,monospace;font-size:10px;color:#5a5570;margin-top:4px">
            ${max(0,td_left):,.0f} buffer remaining · from peak ${apex_status["high_water"]:,.0f}
          </div>
        </div>""", unsafe_allow_html=True)
        ...
        <div class="panel" style="margin-bottom:10px;border-left:2px solid {pt_color}">
          <div style="display:flex;justify-content:space-between;margin-bottom:6px">
            <span style="font-family:Cinzel,serif;font-size:10px;color:#fff;letter-spacing:.08em">PROFIT TARGET</span>
          ...
          <div style="font-family:JetBrains Mono,monospace;font-size:10px;color:#5a5570;margin-top:4px">
            {"★ EVALUATION PASSED" if pt_pct>=100 else f"${pt_left:,.0f} to go · {pt_pct:.0f}% complete"}
          </div>
        </div>""", unsafe_allow_html=True)

        # Trading days
        td_days = apex_status["trading_days"]; td_need = APEX_CONFIG["min_trading_days"]
        td_pct2 = min(100, td_days/td_need*100)
        td_col2 = "#1aff8a" if td_days>=td_need else "#c9a84c"
        st.markdown(f"""
        <div class="panel" style="border-left:2px solid {td_col2}">
          <div style="display:flex;justify-content:space-between;margin-bottom:6px">
            <span style="font-family:Cinzel,serif;font-size:10px;color:#fff;letter-spacing:.08em">TRADING DAYS</span>
          ...
          <div style="font-family:JetBrains Mono,monospace;font-size:10px;color:#5a5570;margin-top:4px">
            {"✓ Minimum days met" if td_days>=td_need else f"{td_need-td_days} more unique trading days needed"}
          </div>
        </div>""", unsafe_allow_html=True)

    # Futures Proxy Table
    st.markdown('<div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.15em;color:var(--apex);margin:24px 0 10px">FUTURES PROXY — WHICH APEX CONTRACT TO EXECUTE</div>', unsafe_allow_html=True)
    proxy_rows=[]
    for mk in SEL:
        sig=market_signals[mk]; m=MARKETS[mk]; cv=council_per_market[mk]; tqs=tqs_per_market[mk]
        risk_1pct = APEX_CONFIG["account_size"]*0.01
        stop_dist = sig.get("price",1)*m["stop"]
        # Rough contracts estimate (simplified: how many units fit $500 risk)
        contracts_est = max(1, int(risk_1pct/max(stop_dist*10,1))) if stop_dist>0 else 1
        proxy_rows.append({
            "Signal":mk,"Direction":sig.get("signal","HOLD"),"TQS":tqs,
            "Council":cv["verdict"],"Apex Contract":m["apex_proxy"],
            "Contract Info":m["apex_note"],"Max Risk 1%":f"${risk_1pct:,.0f}",
            "Est. Contracts":f"1–{min(3,contracts_est)}",
            "Current Price":fmt_price(mk, sig.get("price",0)),
        })
    ...
    daily_hist = st.session_state.get("apex_daily_history",[])
    if len(daily_hist)>=2:
        st.markdown('<div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.15em;color:var(--apex);margin:20px 0 10px">DAILY P&L HISTORY</div>', unsafe_allow_html=True)
        ...
        fig_dh.add_hline(y=0,  line=dict(color="#3a3550",width=1,dash="dot"))
        fig_dh.add_hline(y=-APEX_CONFIG["daily_loss_limit"], line=dict(color="#ff2d55",width=1,dash="dash"),
            annotation_text="Daily Limit", annotation_position="right")
        fig_dh.update_layout(height=260,template="plotly_dark",paper_bgcolor="#05040a",
            plot_bgcolor="#09080f",margin=dict(l=0,r=0,t=10,b=0),showlegend=False,
    ...
    # Apex rules reference
    st.markdown("""
    <div style="background:#0d0c16;border:1px solid #12101e;border-left:2px solid var(--apex);border-radius:1px;padding:16px 20px;margin-top:16px;font-family:Cormorant Garamond,serif;font-size:13px;line-height:1.8;color:#5a5570">
    <div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.15em;color:var(--apex);margin-bottom:10px">APEX $50K RULES REFERENCE</div>
    <b style="color:#c9a84c">Daily Loss Limit — $1,000:</b> If you lose $1,000 in one calendar day (ET), stop trading immediately. NIGEL auto-locks all desks.<br>
    <b style="color:#c9a84c">Trailing Drawdown — $2,500:</b> Your account can never be $2,500 below its highest-ever balance. This trail moves up with profits but never down. Protect your peak equity aggressively.<br>
    <b style="color:#c9a84c">Profit Target — $3,000:</b> Reach $53,000 equity to pass the evaluation. There is no time limit.<br>
    <b style="color:#c9a84c">Min Trading Days — 7:</b> You must trade on at least 7 unique calendar days before withdrawing.<br>
    <b style="color:#c9a84c">Position Sizing:</b> At 1% risk ($500) with a typical 3–4% stop on these volatile instruments, you are sizing for 1–2 contracts maximum on most Apex futures proxies. Do not over-lever.
    </div>""", unsafe_allow_html=True)
...
with t1:
    ...
    for i,(col,mk) in enumerate(zip(tqs_cols,sorted_mk)):
        ...
        with col:
            ...
            <div class="tqs-cell" style="border-top:2px solid {tc}">
              <div style="font-family:JetBrains Mono,monospace;font-size:9px;color:#3a3550;margin-bottom:2px">{rank} RANK {i+1}</div>
              ...
              <div style="margin-top:6px;font-family:JetBrains Mono,monospace;font-size:10px;color:#5a5570">
                ...
                <div style="color:{vc};margin-top:3px;font-weight:700">{cv["verdict"]}</div>
        ...
        for mk in SEL:
            ...
            <div style="margin-bottom:12px">
              ...
              <div style="display:grid;grid-template-columns:1fr 1fr 1fr;font-family:JetBrains Mono,monospace;font-size:9px;margin-top:3px">
                <div style="color:#1aff8a">▲ {cv["long"]} LONG</div>
                <div style="color:#ff2d55">▼ {cv["short"]} SHORT</div>
                <div style="color:#5a5570">— {cv["hold"]} HOLD</div>
              </div>
            </div>""", unsafe_allow_html=True)

    with p1r:
        st.markdown('<div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.15em;color:#c9a84c;margin-bottom:10px">TOP SETUP</div>', unsafe_allow_html=True)
        ...
        <div class="panel" style="border-top:2px solid {bvc}">
          ...
          <div style="font-family:Cormorant Garamond,serif;font-style:italic;color:#5a5570">{m['label']}</div>
          <div style="margin:6px 0"><span class="proxy-tag">{m['apex_proxy']}</span> <span style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:11px;color:#5a5570">{m['apex_note']}</span></div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:10px 0">
            <div><div class="stat-val" style="color:#c9a84c">{btqs}</div><div class="stat-lbl">TQS</div></div>
            <div><div class="stat-val" style="color:{bvc}">{bcv["verdict"]}</div><div class="stat-lbl">Council</div></div>
          </div>
    ...
    race=sorted([(tr["name"],tr["balance"],tr["balance"]-STARTING_BALANCE,tr["emoji"]) for tr in TRADERS],key=lambda x:x[1],reverse=True)
    fig_race=go.Figure(go.Bar(x=[p for _,_,p,_ in race],y=[f"{e} {n}" for n,_,_,e in race],
        orientation="h",marker=dict(color=["#1aff8a" if p>0 else "#ff2d55" for _,_,p,_ in race],opacity=0.8),
        text=[f"${p:+,.0f}" for _,_,p,_ in race],textposition="outside",
        textfont=dict(family="JetBrains Mono",size=11,color="#d4cfc0")))
    fig_race.update_layout(height=240,template="plotly_dark",paper_bgcolor="#05040a",plot_bgcolor="#09080f",
...
with t2:
    ...
    with ni1:
        ...
        colors={"2:08 PMClaude responded: watch":"#c9a84c","buy":"#1aff8a","sell":"#ff2d55","info":"#00c4ff"}watch":"#c9a84c","buy":"#1aff8a","sell":"#ff2d55","info":"#00c4ff"}
for n in notes[:8]:
...
rows.append({"Contract":mk,"Signal":sig,"Conf":f"{sig['conf']}%",
"TQS":tqs_per_market.get(mk,0),"Council":council_per_market["verdict"],
"RSI":f"{sig['rsi']:.0f}","Mom":f"{sig['mom']:+.1f}%",
"Regime":sig.get('regime','?'),"Divergence":divs,
"Apex Proxy":MARKETS["apex_proxy"]})
st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True)
══════════════════════════════════════════════════════════════
TAB 3 — TRADERS
══════════════════════════════════════════════════════════════
with t3:
    st.markdown('<div style="font-family:Cinzel,serif;font-size:10px;letter-spacing:.2em;color:#c9a84c;margin-bottom:16px">THE SIX DESKS — ADAPTIVE LEARNING ACTIVE</div>', unsafe_allow_html=True)
    if apex_status["locked"]:
        st.markdown(f'<div class="shield-banner">🔒 APEX LOCK — {apex_status["lock_reason"]}</div>', unsafe_allow_html=True)
    sb_rows=[]
    for tr in TRADERS:
        pnl=tr["balance"]-STARTING_BALANCE; wins=sum(1 for t in tr["trades"] if t.get("result")=="win")
        ...
        sb_rows.append({"Desk":f"{tr['emoji']} {tr['name']}","Fitness":tr.get("fitness",0),
            "Balance":tr["balance"],"P&L ($)":pnl,"Win%":wr,"Trades":tot,"DD%":dd,
            "Streak":streak,"Best Mkt":tr.get("best_market") or "—",
    ...
    st.dataframe(sb_df.style
        .format({"Balance":"{:,.0f}","P&L (
)":"${:+,.0f}","Win%":"{}%","DD%":"{}%","Fitness":"{:.1f}"})
    ...
    for tab,tr in zip(tr_tabs,TRADERS):
        with tab:
            ...
            ec="#1aff8a" if kelly_info["edge"]>0 else "#ff2d55"
            fit=tr.get("fitness",0); fc="#1aff8a" if fit>=60 else "#c9a84c" if fit>=35 else "#ff2d55"
            adapt_ind=(f'<span class="adaptive-tag">◈ ADAPTED · base {tr.get("base_min_conf",tr["min_conf"])} → {tr["min_conf"]}</span>'
                       if tr["min_conf"]!=tr.get("base_min_conf",tr["min_conf"]) else
                       f'<span style="font-family:JetBrains Mono,monospace;font-size:9px;color:#3a3550">at base ({tr["min_conf"]})</span>')
            ...
            <div class="panel panel-gold" style="margin-bottom:16px">
              ...
              <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:12px">
                ...
                <div><div class="stat-val" style="color:{ec}">{kelly_info['edge']:+.1f}%</div><div class="stat-lbl">Edge</div></div>
              ...
              <div style="font-family:JetBrains Mono,monospace;font-size:9px;color:#3a3550;margin-top:10px">
                Risk {tr['risk_pct']*100:.1f}% · RR {tr['rr']}:1 · Half-Kelly {kelly_info['half_kelly']:.2f}% · {adapt_ind}
              </div>
              {paused_html}
            </div>""", unsafe_allow_html=True)
            pos=tr.get("open_pos")
            if pos and _is_valid_position(pos):
                ...
                is_long=pos.get("dir")=="long"; cls="pos-long" if is_long else "pos-short"
                unr=(cur-pos["entry"])*pos["units"] if is_long else (pos["entry"]-cur)*pos["units"]
                uc="#1aff8a" if unr>=0 else "#ff2d55"; trail=" · 🔒 TRAILED" if pos.get("trailed") else ""
                st.markdown(f'<div class="{cls} pos-panel">{"▲ LONG" if is_long else "▼ SHORT"} — {MARKETS.get(mk,{}).get("label",mk)}{trail} · {pos.get("bars_in_trade",0)} bars<span style="float:right;color:#5a5570">{pos.get("time","")}</span><br>Entry {pos["entry"]:,.2f} · Now ${cur:,.2f} · SL <span style="color:#ff2d55">
{pos["stop"]:,.2f}</span> · TP <span style="color:#1aff8a">pos["tp"]:,.2f</span><br>Unrealized<spanstyle="color:uc;font−weight:700">{pos["tp"]:,.2f}</span><br>Unrealized <span style="color:{uc};font-weight:700">
pos["tp"]:,.2f</span><br>Unrealized<spanstyle="color:uc;font−weight:700">{unr:+,.2f}</span> · Risk ${pos.get("risk_amt",0):,.0f}</div>', unsafe_allow_html=True)
            else:
            ...
            if recent:
                st.markdown('<div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.12em;color:#5a5570;margin:10px 0 4px">RECENT TRADES</div>', unsafe_allow_html=True)
                for t in recent:
                    ...
                    st.markdown(f'<div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid #12101e"><div><span class="{pc}">■</span><span style="font-family:JetBrains Mono,monospace;font-size:10px;color:#d4cfc0;margin-left:8px">{ml} {"▲" if t.get("dir")=="long" else "▼"}</span><span style="font-family:JetBrains Mono,monospace;font-size:9px;color:#5a5570;margin-left:6px">{t.get("reason","")} · proxy:{proxy}</span></div><span style="font-family:JetBrains Mono,monospace;font-size:11px;color:{tc};font-weight:600">${t.get("pnl",0):+,.2f}</span></div>', unsafe_allow_html=True)
    # Equity curves
    ...
    fig_eq.update_layout(height=300,template="plotly_dark",paper_bgcolor="#05040a",plot_bgcolor="#09080f",
...
bc="badge-long" if is_b else "badge-short"
...
<div style="font-family:JetBrains Mono,monospace;font-size:9px;color:#3a3550;min-width:60px;padding-top:2px">{item['time']}</div>
<div style="flex:1">
...
mc_colors=["#1aff8a" if v>=0 else "#ff2d55" for v in macd_vals]
fig.add_trace(go.Bar(x=mx,y=macd_vals,name="MACD",marker=dict(color=mc_colors,opacity=0.4)),row=2,col=1)
sig_x=list(range(26,26+len(macd_signal_vals)))
fig.add_trace(go.Scatter(x=sig_x,y=macd_signal_vals,mode="lines",name="Signal",line=dict(color="#00c4ff",width=1)),row=2,col=1)
...
fig.add_trace(go.Bar(x=x_vals,y=bar_changes,name="ΔClose%",marker=dict(color="#c9a84c",opacity=0.4)),row=4,col=1)
fig.update_layout(height=700,template="plotly_dark",paper_bgcolor="#05040a",plot_bgcolor="#09080f",
...
══════════════════════════════════════════════════════════════
TAB 6 — RULES ENGINE (Apex-aware)
══════════════════════════════════════════════════════════════
with t6:
st.markdown('<div style="font-family:Cinzel,serif;font-size:10px;letter-spacing:.2em;color:#c9a84c;margin-bottom:16px">RULES ENGINE — APEX EDITION</div>', unsafe_allow_html=True)
...
st.session_state["rule_set"].append({"type":"vol_min","value":1.2,"active":True,"name":"Volume Floor","desc":"Require 1.2× avg volume"})
state_save(); st.rerun()
with qp4:
if st.button("⚡ Cap ATR 6%"):
st.session_state["rule_set"].append({"type":"atr_max","value":6.0,"active":True,"name":"ATR Cap","desc":"Block when ATR% > 6% (volatile instruments)"})
state_save(); st.rerun()
with qp5:
if st.button("🔒 Apex $200 Guard"):
st.session_state["rule_set"].append({"type":"apex_daily_guard","value":200,"active":True,"name":"Apex Daily Guard","desc":"Block new trades if <$200 daily headroom"})
state_save(); st.rerun()
st.divider()
...
with cols[2]:
if st.button("✕",key=f"del_{i}"): st.session_state["rule_set"].pop(i); state_save(); st.rerun()
with rl2:
st.markdown('<div style="font-family:Cinzel,serif;font-size:9px;letter-spacing:.15em;color:#5a5570;margin-bottom:8px">RULE IMPACT</div>', unsafe_allow_html=True)
for mk,sig in market_signals.items():
blocked=sig.get("rule_block",False); ic="#ff2d55" if blocked else "#1aff8a"; lbl="BLOCKED" if blocked else "OPEN"
st.markdown(f'<div style="display:flex;justify-content:space-between;padding:6px 12px;background:#0d0c16;border:1px solid #12101e;border-left:2px solid {ic};border-radius:1px;margin-bottom:4px;font-family:JetBrains Mono,monospace;font-size:11px"><span style="color:#fff">{mk}</span><span style="color:{ic};font-weight:600">{lbl}</span></div>', unsafe_allow_html=True)
══════════════════════════════════════════════════════════════
TAB 7 — BACKTEST (Apex daily limit respected)
══════════════════════════════════════════════════════════════
with t7:
st.markdown('<div style="font-family:Cinzel,serif;font-size:10px;letter-spacing:.2em;color:#c9a84c;margin-bottom:8px">INTERACTIVE BACKTEST — APEX RULES APPLIED</div>', unsafe_allow_html=True)
st.markdown('<div style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:13px;color:#5a5570;margin-bottom:16px">Backtest respects Apex daily loss limit — days where $1K is hit get no further trades.</div>', unsafe_allow_html=True)
bc=st.columns(5)
...
if run_bt:
with st.spinner("Running backtest with Apex rules…"):
result=run_backtest_nigel(raw_data[bt_mk]["closes"],bt_mk,bt_risk,bt_rr,
st.session_state["rule_set"] if bt_rules else [])
st.session_state["bt_cache"][f"{bt_mk}{bt_rr}{bt_risk}"]=result; state_save()
if st.session_state["bt_cache"]:
result=list(st.session_state["bt_cache"].values())[-1]
if "error" in result: st.error(result["error"])
else:
r1=st.columns(5); rc="#1aff8a" if result["return_pct"]>0 else "#ff2d55"
with r1[0]: st.markdown(f'<div class="panel panel-gold"><div class="stat-val" style="color:{rc}">${result["total_pnl"]:+,.0f}</div><div class="stat-lbl">Total P&L</div></div>', unsafe_allow_html=True)
with r1[1]: st.markdown(f'<div class="panel"><div class="stat-val" style="color:{rc}">{result["return_pct"]:+.1f}%</div><div class="stat-lbl">Return</div></div>', unsafe_allow_html=True)
...
fig_bt.update_layout(height=280,template="plotly_dark",paper_bgcolor="#05040a",plot_bgcolor="#09080f",
...
══════════════════════════════════════════════════════════════
with t8:
st.markdown('<div style="font-family:Cinzel,serif;font-size:10px;letter-spacing:.2em;color:#c9a84c;margin-bottom:8px">AI ANALYST — APEX AWARE</div>', unsafe_allow_html=True)
if not CLKEY:
...
question=None
with ac1:
if st.button("📋 Full Brief"): question="Give me a complete brief on every instrument. Include which Apex futures contract to use for each and exact risk sizing for a $50K account with $1K daily limit."
with ac2:
if st.button("🎯 Best Trade"): question="What is the single best trade idea right now? Exact entry, stop, target, which Apex contract, and how many contracts at 1% risk on $50K."
with ac3:
if st.button("🛡 Risk Check"): question="Audit my Apex account risk. Check my trailing drawdown, daily P&L, and whether I should still be trading today."
with ac4:
...
ctx=build_ai_context(market_signals,live_prices,diag,apex_status,st.session_state.get("bt_cache"))
...
with et1:
st.markdown('<div style="font-family:Cinzel,serif;font-size:10px;letter-spacing:.2em;color:#c9a84c;margin-bottom:16px">RISK OF RUIN · KELLY ENGINE · APEX CALIBRATED</div>', unsafe_allow_html=True)
...
<div>{"⚠ You are betting OVER Half-Kelly" if kdata["vs_half_kelly"]=="OVER" else "✓ Sized below Half-Kelly — disciplined"}</div>
</div>
</div>""", unsafe_allow_html=True)
with rc2:
...
fig_h.update_layout(height=320,template="plotly_dark",paper_bgcolor="#05040a",plot_bgcolor="#09080f",
...
fig_c.update_layout(height=380,template="plotly_dark",paper_bgcolor="#05040a",plot_bgcolor="#09080f",
margin=dict(l=0,r=0,t=10,b=0),font=dict(family="JetBrains Mono",size=10,color="#5a5570"))
st.plotly_chart(fig_c,use_container_width=True,key="corr_heat")
st.markdown('<div style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:13px;color:#5a5570;margin-top:10px">SOL/AVAX and TQQQ/SOXL are SMT pairs — divergence between them creates high-conviction signals.</div>', unsafe_allow_html=True)
with et3:
...
rn=reg.get("regime","?"); rc="#1aff8a" if rn=="TRENDING" else "#c9a84c" if rn=="RANGING" else "#ff2d55"
fc2=vr.get("forecast","?"); fcc="#1aff8a" if fc2=="CONTRACTING" else "#c9a84c" if fc2=="STABLE" else "#ff2d55"
st.markdown(f"""
<div class="panel" style="margin-bottom:8px;border-left:2px solid {rc}">
<div style="display:flex;justify-content:space-between;align-items:center">
<div><span style="font-family:Cinzel,serif;font-weight:700;color:#fff">{mk}</span>
<span style="font-family:Cormorant Garamond,serif;font-style:italic;color:#5a5570;margin-left:6px">{MARKETS[mk]['label']}</span>
<span class="proxy-tag" style="margin-left:6px">{MARKETS[mk]['apex_proxy']}</span></div>
<span class="badge" style="background:{rc}22;color:{rc};border:1px solid {rc}55">{rn}</span>
</div>
...
<div><div style="font-family:JetBrains Mono,monospace;color:{fcc};font-size:1rem">{fc2}</div><div class="stat-lbl">Vol Forecast</div></div>
</div>
</div>""", unsafe_allow_html=True)
══════════════════════════════════════════════════════════════
...
apex_bar_color="#ff2d55" if apex_status["locked"] else "#ff9500" if apex_status["warnings"] else "#1aff8a"
apex_bar_text=(f"🔒 LOCKED" if apex_status["locked"]
else f"⚠ ${APEX_CONFIG['daily_loss_limit']+apex_status['daily_pnl']:,.0f} daily left" if apex_status["warnings"]
else f"APEX CLEAR · daily ${APEX_CONFIG['daily_loss_limit']+apex_status['daily_pnl']:,.0f} left")
