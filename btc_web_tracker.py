"""
NIGEL — Private Trading Intelligence  v3.1
...
.persist-banner{background:rgba(26,255,138,0.04);border:1px solid rgba(26,255,138,0.15);border-left:2px solid var(--emerald);border-radius:1px;padding:8px 14px;font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--emerald);margin-bottom:12px;display:flex;align-items:center;gap:10px;}
.persist-dot{display:inline-block;width:6px;height:6px;border-radius:50%;background:var(--emerald);margin-right:4px;}
</style>
""", unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════
# ◈ PERSISTENT STATE ENGINE — v3.1
# All data that should survive restarts is stored here.
# The NigelState class handles atomic read/write with versioning
# and graceful migration from older state files.
# ══════════════════════════════════════════════════════════════

PERSIST_PATH = pathlib.Path("nigel_state.json")
STATE_VERSION = "3.1"

# Fields that are persisted to disk (subset of session_state)
PERSIST_KEYS = [
...
def _serialize(obj):
    """JSON-safe serializer — handles datetime, numpy, pandas."""
...
def state_save():
    """Atomically save all persist-worthy session state to disk."""
    payload = {"__version__": STATE_VERSION, "__saved_at__": datetime.now().isoformat()}
    for k in PERSIST_KEYS:
        if k in st.session_state:
            try:
                v = st.session_state[k]
                # Sanitise bt_cache — DataFrames inside need special handling
                if k == "bt_cache":
                    safe = {}
                        ...
                        if isinstance(bv, dict):
                            ...
                            for ek, ev in bv.items():
                                if isinstance(ev, pd.DataFrame):
                                    entry[ek] = {"__dataframe__": ev.to_dict(orient="records")}
                                elif isinstance(ev, list) and ev and isinstance(ev[0], np.floating):
                                    entry[ek] = [float(x) for x in ev]
                                else:
                                    entry[ek] = ev
                            safe[bk] = entry
                        else:
                    ...
                    json.dumps(v, default=_serialize)  # validate first
                    payload[k] = v
            except Exception:
                pass  # skip non-serializable values silently
    try:
        tmp = PERSIST_PATH.with_suffix(".tmp")
        with open(tmp, "w") as f:
            json.dump(payload, f, default=_serialize, indent=2)
        tmp.replace(PERSIST_PATH)  # atomic rename
    except Exception as e:
        pass  # never crash the app over a save failure

def state_load():
    """Load persisted state. Returns dict or {} on any failure."""
    ...
    try:
        ...
        # Restore bt_cache DataFrames
        if "bt_cache" in data:
            for bk, bv in data["bt_cache"].items():
                if isinstance(bv, dict) and "trades" in bv:
                    t = bv["trades"]
                    if isinstance(t, dict) and "__dataframe__" in t:
                        bv["trades"] = pd.DataFrame(t["__dataframe__"])
...
def state_get(key, default=None):
    """Load a single key from disk state (used during init)."""
    return state_load().get(key, default)

# ── One-time restore on cold start ──────────────────────────
# We use a sentinel to avoid re-loading on every Streamlit rerun.
...
# ══════════════════════════════════════════════════════════════
# SESSION STATE — uses persisted values if available
# ══════════════════════════════════════════════════════════════
...
# Patch traders for missing fields (backward compat)
for tr in st.session_state["traders"]:
    if "philosophy" not in tr: tr["philosophy"] = tr.get("style", "")
    if "paused" not in tr:     tr["paused"] = False
    if "history" not in tr:    tr["history"] = [tr.get("balance", 25000.0)]
    if "open_pos" not in tr:   tr["open_pos"] = None
...
def simulate_trader(tr, market_signals):
    ...
    if tr["open_pos"]:
        ...
        if hit_sl or hit_tp:
            ...
            tr["open_pos"] = None
            state_save()  # ← persist after every trade close
    ...
    for mk, sig in market_signals.items():
        ...
        state_save()  # ← persist after every new position
        break
...
def run_backtest_nigel(closes, mk, risk_pct=0.01, rr=2.0, rules=None):
    ...
    result = {"mk":mk,"total_pnl":round(tdf["pnl"].sum(),2),"return_pct":round((bal-cap)/cap*100,2),
...
with st.sidebar:
    ...
    st.markdown('<div style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:11px;color:#5a5570;margin-bottom:20px">Private Trading Intelligence v3.1</div>',unsafe_allow_html=True)
    st.divider()
    ...
    st.markdown(f"""
    <div style="background:rgba(26,255,138,0.04);border:1px solid rgba(26,255,138,0.15);border-left:2px solid #1aff8a;border-radius:1px;padding:8px 12px;margin-bottom:12px">
      <div style="font-family:JetBrains Mono,monospace;font-size:9px;color:#1aff8a;letter-spacing:.1em">◈ STATE PERSISTENCE ACTIVE</div>
      <div style="font-family:JetBrains Mono,monospace;font-size:9px;color:#3a3550;margin-top:3px">Restored at {restore_ts} · {state_file_size/1024:.1f} KB</div>
    ...
    if sel:
        st.session_state["selected_markets"]=sel
        state_save()
    ...
    if st.button("💾 Force Save"):
        state_save()
        st.success(f"Saved to {PERSIST_PATH} ({PERSIST_PATH.stat().st_size/1024:.1f} KB)")
...
if (now_ts-st.session_state["last_refresh"])>=st.session_state["refresh_interval"]:
    ...
    state_save()  # ← persist on every refresh cycle
...
with t6:
    ...
    if not st.session_state["rule_set"]:
        ...
        for i,rule in enumerate(st.session_state["rule_set"]):
            ...
            with rc2:
                if st.button("Toggle",key=f"tog_{rule['id']}"): st.session_state["rule_set"][i]["active"]=not active; state_save(); st.rerun()
            with rc3:
                if st.button("Delete",key=f"del_{rule['id']}"): st.session_state["rule_set"].pop(i); state_save(); st.rerun()
...
with t7:
    ...
    if run_bt:
        with st.spinner("Backtesting…"):
            ...
            state_save()  # ← persist backtest results
        ...
        with ai1:
            ...
            if final_q:
                ...
                if err: st.error(f"Claude error: {err}")
                    ...
                    state_save()  # ← persist AI conversations
        with ai2:
...
with t9:
    ...
    with et1:
        ...
        fig_ror.update_layout(height=260,template="plotly_dark",paper_bgcolor="#05040a",plot_bgcolor="#09080f",margin=dict(l=0,r=0,t=10,b=0),xaxis=dict(title=dict(text="Risk Per Trade",font=dict(family="Cinzel",size=9,color="#5a5570")),gridcolor="#12101e"),yaxis=dict(title=dict(text="Win Rate",font=dict(family="Cinzel",size=9,color="#5a5570")),gridcolor="#12101e"),font=dict(family="JetBrains Mono",size=9,color="#5a5570"))
...
if st.session_state["always_on"]:
    ...
    <div class="always-on-bar">
      <div><span class="live-dot"></span>NIGEL v3.1 · ALWAYS ON · {datetime.now().strftime("%H:%M:%S")}{shield_str}</div>
      <div style="font-family:JetBrains Mono,monospace;font-size:9px;color:#3a3550">◈ STATE {persist_kb:.1f}KB · RESTORED {st.session_state.get("__restore_ts__","—")}</div>
    ...
    if now_ts2-st.session_state["last_refresh"]>=interval:
        ...
        state_save()  # ← persist on every auto-refresh
